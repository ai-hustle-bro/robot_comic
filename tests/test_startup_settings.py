...
def test_write_and_read_startup_settings(tmp_path) -> None:
    ...
    write_startup_settings(tmp_path, profile="default", voice="shimmer")

    assert read_startup_settings(tmp_path) == StartupSettings(profile="default", voice="shimmer")
...
def test_partial_update_preserves_other_fields(tmp_path) -> None:
    ...
    assert read_startup_settings(tmp_path) == StartupSettings(profile="default", voice="shimmer", movement_speed=0.45)
...
def test_explicit_none_clears_field(tmp_path) -> None:
    ...
    write_startup_settings(tmp_path, profile="default", voice="shimmer", movement_speed=0.45)
...
def test_load_startup_settings_into_runtime_applies_profile_when_no_env(monkeypatch, tmp_path) -> None:
    ...
    assert settings == StartupSettings(profile="default", voice="shimmer")
...
def test_load_startup_settings_into_runtime_saved_settings_override_inherited_env(monkeypatch, tmp_path) -> None:
    ...
    write_startup_settings(tmp_path, profile="don_rickles", voice="cedar")
    applied_profiles: list[str | None] = []
    ...
    assert settings == StartupSettings(profile="don_rickles", voice="cedar")
    assert applied_profiles == ["don_rickles"]
