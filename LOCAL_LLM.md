...
Note: Using port 8080 to match `robot_comic.config.LLAMA_CPP_DEFAULT_URL`.
Set `LLAMA_CPP_URL` explicitly if an older workstation service still listens on 11434.
...
The old `_ollama_base_url` property in `chatterbox_tts.py` derived the LLM URL
from `CHATTERBOX_URL` by swapping ports — fragile. Add a dedicated var:
